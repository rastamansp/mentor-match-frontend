...
import { Event, Ticket, Payment } from '../types'
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [eventsData, ticketsData, paymentsData] = await Promise.all([
          eventsApi.getAll(),
          ticketsApi.getAll({ userId: user.id }),
          paymentsApi.getAll({ userId: user.id })
        ])
