...
export const EditProductPage: React.FC = () => {
  ...
  const { product, loading: loadingProduct, error: errorProduct } = useProductDetail(id || '')
  ...
  // Preencher formulário quando o produto for carregado
  useEffect(() => {
    if (product) {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isOrganizer) {
      toast.error('Você não tem permissão para editar produtos neste evento')
      return
    }

    try {
      const dataToSend: Partial<CreateProductDto> = {
  ...
  if (loadingEvent || loadingProduct) {
    return (
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Erro ao carregar produto</h2>
          <p className="text-gray-600 mb-4">{errorProduct.message}</p>
          <Link
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">Você não tem permissão para editar produtos neste evento.</p>
          <Link
            to={`/events/${eventId}/products`}
            className="btn-primary"
          >
